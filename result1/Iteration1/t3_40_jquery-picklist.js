...
(function($)
{
	$.widget("awnry.pickList",
	{
		...
		options:
		{
			...
			// cross-scripting options
			// By default xss will use .html() to display text, but this strips out html tags, sometimes 
			//   these tags are needed.  In this cases, .text() should be used instead, but only if
			//   the developer is sure no bad xml tags will be displayed
			xss:                        true
		},
		...
		_buildSourceList: function()
		{
			...
			self.sourceList = $("<ul/>")
					...
					.prop("tabindex", "1")
					.selectable({ cancel: '.ui-selected' }) // re-enables dblclick event as a side-effect
					.on( "selectableselecting", function( event, ui ) { self._refreshControls(); })
					.on( "selectableunselecting", function( event, ui ) { self._refreshControls(); })
					.delegate("", "mouseup", { pickList: self }, self._changeHandler);
			...
			self.sourceList.delegate(".pickList_listItem", "dblclick", {pickList: self}, function(e)
			{
				...
				if (self.options.selectLimit > self.targetList.children().length) {
					self._addItems( self.sourceList.children(".ui-selected") );
				}
			});
			
			self.sourceList.on("keypress", self._keypressHandler);
			self.sourceList[0].matchString = "";

			if (self.element.hasClass("form-element-error")) {
				self.sourceList.addClass(self.options.errorClass);
			}
		...
		_buildTargetList: function()
		{
			...
			self.targetList.on("keypress", self._keypressHandler);
			self.targetList[0].matchString = "";

			if (self.element.hasClass("form-element-error")) {
				self.targetList.addClass(self.options.errorClass);
			}
		...
		_buildControls: function()
		{
			...
			if (self.options.enableAddAll) {
				self.controls.append(self.addAllButton);
			}
			self.controls.append(self.addButton);
			self.controls.append(self.removeButton);
			if (self.options.enableRemoveAll) {
				self.controls.append(self.removeAllButton);
			}
		...
		_addAllHandler: function(e)
		{
			...
			self.element.children().not(":selected").prop("selected", true);
		...
		{
			var self = e.data.pickList;
			var itemsSelectedCount = self.sourceList.children(".ui-selected").length;
			var itemsAlreadyTargettedCount = self.targetList.children().length;
			var spacesAvailableCount = self.options.selectLimit - itemsAlreadyTargettedCount;
			if (self.options.selectLimit > itemsAlreadyTargettedCount && spacesAvailableCount >= itemsSelectedCount) {
				self._addItems(self.sourceList.children(".ui-selected"));
			}
		},
		...
		_keypressHandler: function(e) {
			...
			var testStr = this.matchString + c;
			var li = $(this).children('li[label^="'+testStr+'"]').first();
			if (li.size() == 0) {
				testStr = this.matchString + c.toUpperCase();
				li = $(this).children('li[label^="'+testStr+'"]').first();
			}
			if (li.size() > 0) {
				$(this).children("li").removeClass("ui-selected");
				li.addClass("ui-selected");
				$(this).scrollTo(li);
			}

			var self = this;
			this.matchString = testStr;
			clearTimeout(this.timerId);
			this.timerId = setTimeout(function() {self.matchString = "";}, 500);
		},
		...
		_refreshControls: function()
		{
			...
			var selectedSourceCount = self.sourceList.children(".ui-selecting, .ui-selected").length;
			var targetCount = self.targetList.children().length;
			if(selectedSourceCount && self.options.selectLimit >= (selectedSourceCount + targetCount))
			{
			...
			if(self.targetList.children(".ui-selecting, .ui-selected").length)
			{
		...
		_createDoppelganger: function(item)
		{
			var self = this;
			if (self.options.xss) {				
				return "<li " + self.options.listItemValueAttribute + "='" + $(item).val() + "' label='" + $(item).html() + "' class='" + self.options.listItemClass + "'>" + $(item).html() + "</li>";
			}
			else {
